...
// pingdotgg/t3code@b0b7b38 packages/shared/src/searchRanking.ts
...
export function scoreSubsequenceMatch(value: string, query: string): number | null {
  ...
  for (let valueIndex = 0; valueIndex < value.length; valueIndex += 1) {
    ...
    if (queryIndex === query.length) {
      ...
      const lengthPenaltyValue = Math.min(64, value.length - query.length);
...
// `value` and `query` must be pre-normalized (trimmed + lowercased).
