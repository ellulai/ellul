...
// pingdotgg/t3code@b0b7b38 apps/web/src/store.ts
...
import { ProviderKind } from "@ellul.ai/types";
import type { ThreadId, TurnId } from "@ellul.ai/types";
import { Schema } from "effect";
import { resolveModelSlugForProvider } from "../lib/shared-model";
...
const EMPTY_SANDBOX: OrchestrationSandbox = {
  pinnedModelByProvider: {},
  updatedAt: EPOCH_ZERO_ISO,
};
...
// Single-origin stub for upstream's resolveEnvironmentHttpUrl from
// environments/runtime/catalog.ts. Ellul is single-env; the multi-env
// registry layer (environments/runtime/service.ts, ~1095 LOC) is not ported.
function resolveEnvironmentHttpUrl(input: {
  readonly environmentId: EnvironmentId;
  readonly pathname: string;
  readonly searchParams?: Record<string, string>;
}): string {
  const origin = typeof location !== "undefined" ? location.origin : "";
  const url = new URL(input.pathname, origin || "http://localhost");
...
function mapThreadShell(
  ...
  const summary: SidebarThreadSummary = {
    ...
    branch: thread.branch,
    worktreePath: thread.worktreePath,
    viewScope: thread.viewScope ?? null,
...
function syncEnvironmentShellSnapshot(
  ...
  const hasExistingThreads = state.threadIds.length > 0;
  const snapshotHasThreads = snapshot.threads.length > 0;
  let nextState: EnvironmentState = {
    ...state,
    ...buildProjectState(nextProjects),
    threadIds: !snapshotHasThreads && hasExistingThreads ? state.threadIds : [],
    threadIdsByProjectId: !snapshotHasThreads && hasExistingThreads ? state.threadIdsByProjectId : {},
    threadShellById: !snapshotHasThreads && hasExistingThreads ? state.threadShellById : {},
    threadSessionById: !snapshotHasThreads && hasExistingThreads ? state.threadSessionById : {},
    threadTurnStateById: !snapshotHasThreads && hasExistingThreads ? state.threadTurnStateById : {},
    sidebarThreadSummaryById: !snapshotHasThreads && hasExistingThreads ? state.sidebarThreadSummaryById : {},
...
function applyEnvironmentOrchestrationEvent(
  ...
  switch (event.type) {
    ...
    case "sandbox.model-pinned":
      return {
        ...state,
        sandbox: {
          pinnedModelByProvider: {
            ...state.sandbox.pinnedModelByProvider,
    ...
    case "sandbox.model-unpinned": {
      const next = { ...state.sandbox.pinnedModelByProvider };
      delete next[event.payload.provider];
      return {
        ...state,
        sandbox: {
          pinnedModelByProvider: next,
          updatedAt: event.payload.updatedAt,
        },
      };
    }
...
// Newest-first ordering for the sidebar. `threadIdsByProjectId` is append-only
// (creation order, oldest first), so we sort at the read boundary by
// latest-activity-first, falling back to createdAt for threads with no turns yet.
function compareSidebarThreadsNewestFirst(
  a: SidebarThreadSummary,
  b: SidebarThreadSummary,
): number {
  const aActivity = a.latestUserMessageAt ?? a.updatedAt ?? a.createdAt;
  const bActivity = b.latestUserMessageAt ?? b.updatedAt ?? b.createdAt;
...
export function selectSidebarThreadsForProjectRef(
  ...
  return threadIds
    ...
    .toSorted(compareSidebarThreadsNewestFirst);
}
...
export function selectSidebarThreadsForProjectRefs(
  ...
  return refs
    .flatMap((ref) => selectSidebarThreadsForProjectRef(state, ref))
    .toSorted(compareSidebarThreadsNewestFirst);
}
...
/**
 * Adjacent `thread.message-sent` deltas for the same messageId collapse into
 * one event before hitting the reducer — a pure perf win on heavy streaming
 * where N delta events would otherwise trigger N reducer passes and N React
 * re-renders. Ported from upstream `environments/runtime/service.ts`
...
function applyOptimisticSandboxPin(
  state: AppState,
  environmentId: EnvironmentId,
  ...
  const environmentState = getStoredEnvironmentState(state, environmentId);
  const next = { ...environmentState.sandbox.pinnedModelByProvider };
  if (model === null) {
...
export const useStore = create<AppStore>((set) => ({
  ...
  applyOptimisticSandboxPin: (environmentId, provider, model) =>
    set((state) => applyOptimisticSandboxPin(state, environmentId, provider, model)),
