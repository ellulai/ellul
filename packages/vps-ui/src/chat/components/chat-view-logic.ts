...
// pingdotgg/t3code@b0b7b38 apps/web/src/components/ChatView.logic.ts.
...
import { selectThreadByRef, useStore } from "../state/store";
import {
...
export const LAST_INVOKED_SCRIPT_BY_PROJECT_KEY = "ellul:last-invoked-script-by-project";
export const MAX_HIDDEN_MOUNTED_TERMINAL_THREADS = 10;

export const LastInvokedScriptByProjectSchema = Schema.Record(ProjectId, Schema.String);

export function shouldWriteThreadErrorToCurrentServerThread(input: {
  serverThread: { environmentId: EnvironmentId; id: ThreadId } | null | undefined;
...
export function revokeUserMessagePreviewUrls(message: ChatMessage): void {
  ...
  for (const attachment of message.attachments) {
    revokeBlobPreviewUrl((attachment as { previewUrl?: string }).previewUrl);
  }
}
...
export function deriveLockedProvider(input: {
  ...
  if (!threadHasStarted(input.thread)) return null;
...
export async function waitForStartedServerThread(
  ...
  if (threadHasStarted(getThread())) return true;
  return await new Promise<boolean>((resolve) => {
    ...
    const unsubscribe = useStore.subscribe((state) => {
      if (!threadHasStarted(selectThreadByRef(state, threadRef))) return;
      finish(true);
    });
...
export function hasServerAcknowledgedLocalDispatch(input: {
  ...
  if (!input.localDispatch) return false;
  ...
  if (input.phase === "running") {
    if (!latestTurnChanged) return false;
    if (latestTurn?.startedAt === null || latestTurn === null) return false;
    if (
