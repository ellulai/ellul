...
// pingdotgg/t3code@b0b7b38 packages/contracts/src/settings.ts

import { Effect, Schema, SchemaTransformation } from "effect";
import { TrimmedNonEmptyString, TrimmedString } from "./ids";
import {
  ClaudeModelOptions,
  CodexModelOptions,
  CursorModelOptions,
...
export const ZeroClawSettings = Schema.Struct({
  enabled: Schema.Boolean.pipe(Schema.withDecodingDefault(Effect.succeed(true))),
  binaryPath: makeBinaryPathSetting("zeroclaw"),
  customModels: Schema.Array(Schema.String).pipe(Schema.withDecodingDefault(Effect.succeed([]))),
});
export type ZeroClawSettings = typeof ZeroClawSettings.Type;
...
export const ServerSettings = Schema.Struct({
  ...
  providers: Schema.Struct({
    ...
    zeroclaw: ZeroClawSettings.pipe(Schema.withDecodingDefault(Effect.succeed({}))),
    grokAgent: GrokSettings.pipe(Schema.withDecodingDefault(Effect.succeed({}))),
  }).pipe(Schema.withDecodingDefault(Effect.succeed({}))),
...
const ZeroClawModelOptionsPatch = Schema.Struct({});
void ZeroClawModelOptions; // schema referenced for parity with other patches
...
const ModelSelectionPatch = Schema.Union([
  ...
  Schema.Struct({
    provider: Schema.optionalKey(Schema.Literal("zeroclaw")),
    model: Schema.optionalKey(TrimmedNonEmptyString),
    options: Schema.optionalKey(ZeroClawModelOptionsPatch),
  }),
  Schema.Struct({
    provider: Schema.optionalKey(Schema.Literal("grokAgent")),
    model: Schema.optionalKey(TrimmedNonEmptyString),
  }),
]);
...
const ZeroClawSettingsPatch = Schema.Struct({
  enabled: Schema.optionalKey(Schema.Boolean),
  binaryPath: Schema.optionalKey(Schema.String),
  customModels: Schema.optionalKey(Schema.Array(Schema.String)),
});
...
export const ServerSettingsPatch = Schema.Struct({
  ...
  providers: Schema.optionalKey(
    Schema.Struct({
      ...
      zeroclaw: Schema.optionalKey(ZeroClawSettingsPatch),
      grokAgent: Schema.optionalKey(GrokSettingsPatch),
    }),
  ),
});
