...
export interface AuditEvent {
  ...
  /**
   * Sandbox this event belongs to — populates the dedicated `sandbox_id`
   * column so reviewers can filter `SELECT ... WHERE sandbox_id = ?`
   * without parsing the JSON `details`. Omit for server-wide events
   * (auth ceremonies, session rotations, etc.).
   */
...
export function logAuditEvent(event: AuditEvent): AuditEntry {
  ...
  db.prepare(
    'INSERT INTO audit_log (timestamp, event, ip, fingerprint, credential_id, session_id, sandbox_id, details, prev_hash, hash) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
  ).run(
    entry.timestamp,
    entry.event,
    entry.ip,
