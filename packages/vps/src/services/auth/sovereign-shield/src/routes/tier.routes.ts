...
import { getCurrentTier, executeTierSwitch } from "../application/gates/Tier";
import { validateInternalToken } from "../application/credentials/InternalToken";
...
export function registerTierRoutes(app: Hono): void {
   ...
   * - ellul-downgrade (bash script via curl to localhost)
  ...
  app.post("/_auth/tier/switch", async (c) => {
    ...
    // Phase 2: Require internal service token (prevents agent from calling directly)
    if (!validateInternalToken(c.req.header("Authorization"))) {
      console.warn(
        "[shield] Rejected tier switch: missing/invalid internal token",
      );
    ...
    const validTiers: SecurityTier[] = ["standard", "web_locked", "private_locked"];
    ...
    console.log(
      `[shield] Tier switch requested: ${currentTier} -> ${targetTier} (source: ${source || "unknown"})`,
    );
    ...
    if (targetTier === "web_locked") {
      ...
      const hasPasskey =
        (
          db.prepare("SELECT COUNT(*) as c FROM credential").get() as {
            c: number;
          }
        ).c > 0;
      if (!hasPasskey) {
    ...
    // Validate tier transition against policy (single source of truth)
    const { validateTransition } = await import("../application/gates/TierPolicy");
    const transition = validateTransition(currentTier, targetTier);
    if (!transition.allowed) {
      return c.json(
        { error: transition.reason, code: transition.code },
        403,
      );
    }

    try {
      await executeTierSwitch(
        targetTier as SecurityTier,
        ipAddress || ip,
        userAgent || "internal",
      );

      console.log(
        `[shield] Tier switch completed: ${currentTier} -> ${targetTier}`,
      );
  ...
  app.get("/_auth/tier/current", (c) => {
    ...
    // Phase 2: Require internal service token
    if (!validateInternalToken(c.req.header("Authorization"))) {
      return c.json({ error: "Unauthorized — internal token required" }, 401);
    }
    ...
    const hasSshKeys =
      fs.existsSync(SSH_AUTH_KEYS_PATH) &&
    ...
    return c.json({
      ...
      canSwitchTo: {
        ...
        web_locked: tier !== "web_locked" && tier !== "private_locked" && hasPasskeys,
        // private_locked requires explicit opt-in via the console Privacy Lock flow.
        // All volumes are LUKS-encrypted by default — cannot infer from /dev/mapper/luks-home.
        // The console flow sets the tier via the API, not via VPS-side tier switching.
        private_locked: false, // Only available through API encryption init flow
      },
    });
  });
}
