...
import {
  TERM_PROXY_PORT,
  SOVEREIGN_SHIELD_PORT,
  AGENT_BRIDGE_PORT,
} from '../../shared/ports';

// Terminal auth proxy. HTTP=ttyd UI only (no auth); WebSocket=terminal token via sovereign-shield.
// Sessions dynamic (claude-<ts> → port from agent-bridge). Port from shared/ports.ts registry.
export function getTermProxyScript(): string {
  return `#!/usr/bin/env node
const http = require('http');
const { WebSocket, WebSocketServer } = require('ws');

const PORT = ${TERM_PROXY_PORT};
const SHIELD_URL = 'http://127.0.0.1:${SOVEREIGN_SHIELD_PORT}';
const BRIDGE_URL = 'http://127.0.0.1:${AGENT_BRIDGE_PORT}';
...
const _consoleOrigin = (() => {
  try { return require('fs').readFileSync('/etc/ellul/console-origin', 'utf8').trim(); }
  catch { return ''; }
})();
const _platformZone = (() => {
  try { return require('fs').readFileSync('/etc/ellul/platform-zone', 'utf8').trim(); }
  catch { return ''; }
})();

function allowedTerminalOrigin(req) {
  const originRaw = req.headers.origin;
  const origin = Array.isArray(originRaw) ? originRaw[0] : originRaw;
  if (!origin || typeof origin !== 'string') return null;
  const hostRaw = req.headers.host;
  const host = Array.isArray(hostRaw) ? hostRaw[0] : hostRaw;
  const selfOrigin = host && typeof host === 'string' ? 'https://' + host : null;
  const allowed = new Set();
  if (_consoleOrigin) allowed.add(_consoleOrigin);
  if (_platformZone) {
    allowed.add('https://' + _platformZone);
    allowed.add('https://www.' + _platformZone);
  }
...
function parseSessionType(sessionId) {
  const match = sessionId.match(/^(main|opencode|claude|codex|cursor)(-\\d+)?$/);
...
const httpServer = http.createServer(async (req, res) => {
  ...
  if (req.method === 'OPTIONS' && req.url && req.url.match(/^\\/term\\/[^/]+\\/capture/)) {
    const origin = allowedTerminalOrigin(req);
    if (!origin) {
      // Reject cross-origin preflights from unknown origins outright —
      // prevents credentialed-CORS reflection on a sensitive endpoint.
      res.writeHead(403);
      res.end();
      return;
    }
  ...
  if (captureMatch && req.method === 'GET') {
    ...
    const allowedOrigin = allowedTerminalOrigin(req);
...
export function getTermProxyService(svcUser: string = "dev"): string {
  ...
  return `[Unit]
Description=ellul Terminal Auth Proxy
After=network.target ellul-luks-boot.service ellul-sovereign-shield.service
...
User=${svcUser}
Group=${svcUser}
Environment=NODE_PATH=/opt/ellul/auth/node_modules:${svcHome}/.node/lib/node_modules
ExecStart=${svcHome}/.node/bin/node /usr/local/bin/ellul-term-proxy
Restart=always
RestartSec=3
