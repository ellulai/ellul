...
// pingdotgg/t3code@b0b7b38 apps/server/src/persistence/Layers/ProjectionCheckpoints.ts

import { OrchestrationCheckpointFile } from "@ellul.ai/types";
import { Effect, Layer, Option, Schema, Struct } from "effect";
import * as SqlClient from "effect/unstable/sql/SqlClient";
import * as SqlSchema from "effect/unstable/sql/SqlSchema";

import { toPersistenceDecodeError, toPersistenceSqlError } from "../Errors";
import {
  DeleteProjectionCheckpointsByThreadInput,
  GetProjectionCheckpointByThreadAndTurnCountInput,
  ListProjectionCheckpointsByThreadInput,
  ProjectionCheckpoint,
  ProjectionCheckpointRepository,
  type ProjectionCheckpointRepositoryShape,
} from "../Services/ProjectionCheckpoints";
...
const makeProjectionCheckpointRepository = Effect.gen(function* () {
  ...
  const clearCheckpointConflict = SqlSchema.void({
    Request: GetProjectionCheckpointByThreadAndTurnCountInput,
  ...
  const listProjectionCheckpointRows = SqlSchema.findAll({
    Request: ListProjectionCheckpointsByThreadInput,
  ...
  const deleteProjectionCheckpointRows = SqlSchema.void({
    Request: DeleteProjectionCheckpointsByThreadInput,
    execute: ({ threadId }) =>
      sql`
        UPDATE projection_turns
        SET
