...
// pingdotgg/t3code@b0b7b38 apps/server/src/persistence/NodeSqliteClient.ts
...
export const TypeId: TypeId = "~local/ellul/SqliteClient";

export type TypeId = "~local/ellul/SqliteClient";
...
// node:sqlite `StatementSync.columns()` ships in Node 22.16.0 / 23.11.0 / >=24.
...
const make = (
  ...
  makeWithDatabase(
    ...
    () => {
      ...
      // WAL: concurrent readers don't block on writers. Without this every
      // getShellSnapshot() raced the projector's writes and surfaced
      // SQLITE_BUSY as an empty thread sidebar on WS reconnect.
      // busy_timeout: retry locked statements for 5s before erroring.
      // synchronous=NORMAL: still crash-durable under WAL, faster than FULL.
      // foreign_keys: orchestration schema relies on FK cascade.
      //
      // Verify journal_mode took (PRAGMA succeeds silently if SQLite refuses
      // the upgrade — e.g. db opened by another process in delete mode). If
      // verification fails, log to stderr so it shows up in journalctl, not
      // swallowed; the bridge keeps starting because partial progress is
      ...
      if (!options.readonly) {
        try {
          ...
          if (row?.journal_mode?.toLowerCase() !== "wal") {
            ...
            console.error(
              `[NodeSqliteClient] journal_mode=WAL refused for ${options.filename}; got ${row?.journal_mode}. Snapshot reads will block on concurrent writes.`,
            );
          }
        } catch (err) {
          // eslint-disable-next-line no-console
          console.error(
            `[NodeSqliteClient] PRAGMA setup failed for ${options.filename}: ${(err as Error)?.message ?? err}`,
          );
        }
      }
      return db;
