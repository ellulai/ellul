...
// pingdotgg/t3code@b0b7b38 apps/server/src/provider/Layers/ClaudeAdapter.ts
...
import { applyClaudePromptEffortPrefix, resolveEffort, trimOrNull } from "../../shared/model";
import {
  Cause,
  Context,
  DateTime,
...
interface ClaudeSessionContext {
  ...
  // Tracks the SDK's currently-applied permission mode so we can skip
  // redundant query.setPermissionMode calls — those write a control
  // message to claude's stdin and await a response, which can hang on
  // the first turn before claude is ready to ack control messages.
  ...
  stopped: boolean;
  /** Set whenever the SDK iterator yields a message; used by the stall
   *  watchdog to detect a wedged Pro spawn (claude crashed silently or
   *  stdio detached) without waiting for an explicit error. */
  lastSdkMessageAt: number;
  stallWatchdogTimer: NodeJS.Timeout | undefined;
}
...
// Auth-state mutation has been extracted to the greenfield Claude OAT
// credential subsystem. The OAT lives in /etc/ellul/shield-data/claude-oat.json
// (root:shield 640) — bridge has no read or write path to it. This adapter:
//
//   - Mints a single-use issuance token before each spawn (issueOat).
//   - Reports observed upstream 401s for AUDIT-ONLY (reportUnauth401).
//
// Heuristic invalidation based on assistant text or stream-cause text is
// forbidden by design and CI-lint-enforced. The shield-side probe loop
// (claude-oat-probe.service.ts) is the SOLE state-mutation signal. See
// docs/v2/security/14-claude-oat-credentials.md for the full threat model.
...
const makeClaudeAdapter = Effect.fn("makeClaudeAdapter")(function* (
  ...
  const handleResultMessage = Effect.fn("handleResultMessage")(function* (
    ...
    if (status === "failed") {
      const apiStatus = "api_error_status" in message ? Number(message.api_error_status) : 0;
      if (apiStatus === 401) {
        // AUDIT-ONLY — never mutate token state from a single 401 here. The
        // call records a `report-401` audit entry in shield's hash-chained
        // log AND triggers an immediate out-of-cycle probe so user-visible
        // login UI surfaces fast on a real revocation. The probe's verified
        // 401 (counted via the quorum window) is what actually moves state.
        // Free-form text matching has been removed entirely; that was the
        // bug class that wiped valid tokens on adversarial assistant prose.
        void getClaudeOatBridgeModule().reportUnauth401({
          threadId: context.session.threadId,
  ...
  const runSdkStream = (context: ClaudeSessionContext): Effect.Effect<void, Error> =>
    Stream.fromAsyncIterable(context.query, (cause) =>
      ...
      Stream.runForEach((message) => {
        const now = Date.now();
        const sinceLastMs = now - context.lastSdkMessageAt;
        context.lastSdkMessageAt = now;
        // Per-message tick — what type, how long since the prior one.
        // Critical for diagnosing wedged Pro turns: if no ticks fire
        // for ≥SDK_STALL_THRESHOLD_MS the watchdog flags it.
        logEvent("claude.sdk.message.tick", {
          threadId: context.session.threadId,
          messageType: (message as { readonly type?: string }).type ?? "unknown",
          sinceLastMs,
          turnId: context.turnState?.turnId ?? null,
        });
        return handleSdkMessage(context, message);
      }),
    );

  const SDK_STALL_THRESHOLD_MS = 60_000;
  const SDK_STALL_KILL_MS = 180_000;
  const SDK_STALL_CHECK_INTERVAL_MS = 15_000;

  const startStallWatchdog = (
    context: ClaudeSessionContext,
    runFork: (effect: Effect.Effect<void, never>) => Fiber.Fiber<void, never>,
  ): void => {
    if (context.stallWatchdogTimer) return;
    context.stallWatchdogTimer = setInterval(() => {
      if (context.stopped) {
        if (context.stallWatchdogTimer) {
          clearInterval(context.stallWatchdogTimer);
          context.stallWatchdogTimer = undefined;
        }
        return;
      }
      if (!context.turnState) return;
      const idleMs = Date.now() - context.lastSdkMessageAt;
      if (idleMs >= SDK_STALL_KILL_MS) {
        logEvent("claude.sdk.stall-kill", {
          threadId: context.session.threadId,
          turnId: context.turnState.turnId,
          idleMs,
          thresholdMs: SDK_STALL_KILL_MS,
        });
        if (context.stallWatchdogTimer) {
          clearInterval(context.stallWatchdogTimer);
          context.stallWatchdogTimer = undefined;
        ...
        runFork(
          emitRuntimeError(context, "Claude turn killed: no SDK messages for 3 minutes.").pipe(
            Effect.flatMap(() => completeTurn(context, "failed", "Turn stalled — no response from Claude process.")),
            Effect.flatMap(() => stopSessionInternal(context, { emitExitEvent: true })),
            Effect.catchAll(() => Effect.void),
          ),
        );
        return;
      }
      if (idleMs >= SDK_STALL_THRESHOLD_MS) {
        logEvent("claude.sdk.stalled", {
          threadId: context.session.threadId,
          turnId: context.turnState.turnId,
          idleMs,
          thresholdMs: SDK_STALL_THRESHOLD_MS,
        });
      }
    }, SDK_STALL_CHECK_INTERVAL_MS);
    if (context.stallWatchdogTimer.unref) context.stallWatchdogTimer.unref();
  };
  ...
  const handleStreamExit = Effect.fn("handleStreamExit")(function* (
    ...
    if (Exit.isFailure(exit)) {
      if (isClaudeInterruptedCause(exit.cause)) {
        ...
        // No auth-mutation from free-form stream-exit cause text. Real auth
        // failures surface via result messages (handled in handleResultMessage
        // with apiStatus === 401) and ultimately through shield-side probe
        // quorum. CI lint forbids any reintroduction of text-based triggers.
  ...
  const startSession: ClaudeAdapterShape["startSession"] = Effect.fn("startSession")(
    function* (input) {
      ...
      const prompt = Stream.fromQueue(promptQueue).pipe(
        ...
        // Log every item the SDK pulls from the queue so we can see whether the
        // iterator is being consumed at all. If we observe `claude.turn.queue.ok`
        // events without matching `claude.turn.consume` events, the SDK has
        // stopped pulling and the stream is wedged (this is the "234 bytes
        // unread" case where claude only ever got the SDK's init message).
        Stream.tap((item) =>
          Effect.sync(() => {
            const text =
              typeof item.message === "object" &&
              item.message !== null &&
              "message" in item.message &&
              typeof (item.message as { message?: unknown }).message === "object"
                ? "user-message"
                : item.message?.type ?? "unknown";
            logEvent("claude.turn.consume", { tag: text });
          }),
        ),
      ...
      const extraArgs: Record<string, string | null> = {
        ...
        // ellul-only: force `--dangerously-skip-permissions` for claude-code's
        // INTERNAL bwrap sandbox regardless of runtime mode. Exact same rationale
        // as codex's `sandbox: "danger-full-access"` (session-runtime.ts):
        //
        //   claude-code sandboxes tool execution via bubblewrap which needs
        //   unshare(2) and mount(2) — both blocked by our seccomp-BPF filter.
        //   bwrap therefore traps out (SIGTRAP / exit 133) on every turn.
        //
        //   The SDK's `sandbox: { enabled: false }` option serializes to
        //   `--settings '{"sandbox":{"enabled":false}}'` which the CLI binary
        //   does not respect for bwrap activation. The only first-class flag
        //   that reliably disables bwrap is `--dangerously-skip-permissions`.
        //
        //   The OUTER per-project namespace already provides the security
        //   boundary: seccomp-BPF, AppArmor, mount/PID/net NS, iptables
        //   egress allowlist, NoNewPrivileges. claude-code's inner bwrap is
        //   redundant double-sandboxing — Anthropic's own SDK documents
        //   `sandbox: { enabled: false }` as "intended for environments that
        //   are externally sandboxed", which is exactly us.
        "dangerously-skip-permissions": null,
      };

      // Pre-compute the sandbox/project id from cwd. Required for both the
      // namespace wrapper (ELLUL_NS_PROJECT) and the OAT issuance audit trail.
      // Throwing here surfaces the spawn refusal before we mint a token.
      const sandboxId = (() => {
        try {
          return sandboxIdFromCwd(input.cwd);
        } catch (cause) {
          throw new ProviderAdapterValidationError({
            provider: PROVIDER,
            operation: "startSession",
            issue: `Claude startSession requires a sandbox-scoped cwd so the process enters its namespace. Received: ${input.cwd ?? "<none>"}`,
            cause,
          });
        }
      })();
      ...
      if (!launchEnv) {
        return yield* new ProviderAdapterProcessError({
          ...
          detail:
            ...
            "User-visible login UI surfaces via the periodic peek poll.",
        });
      }
      ...
      const queryOptions: ClaudeQueryOptions = {
        ...
        pathToClaudeCodeExecutable: launchEnv.launcherPath,
        ...
        ...(permissionMode === "bypassPermissions"
          ...
          : {}),
        // Belt-and-suspenders: the SDK option does NOT reliably disable
        // bwrap in current claude-code binaries (the --settings JSON path
        // is ignored). The real control is `--dangerously-skip-permissions`
        // injected via extraArgs above. Keep this so it takes effect if
        // upstream fixes the settings path.
          ...
          return {
            mcpServers: {
              "ellul-platform": {
                ...
                args: [] as string[],
                env: {
                  ELLUL_MCP_URL: relay.mcpUrl,
                  ELLUL_MCP_TOKEN: relay.token,
                  ELLUL_MCP_PROJECT: relay.projectName,
                },
              },
            },
          };
        ...
        systemPrompt: {
          ...
          append: `\n\n${buildSystemInstructions("default")}`,
        },
        ...
        canUseTool,
        // ellul-only: the Claude Agent SDK spawns `claude` via Node's
        // child_process.spawn — bypasses our Effect ChildProcessSpawner layer.
        // Bridge redeems the OAT in-process and passes it via env to
        // ellul-claude-ns, which enters the namespace and execs claude.
        env: {
          ...launchEnv.env,
          ...(input.cwd ? { ELLUL_NS_CWD: input.cwd } : {}),
        },
        ...(input.cwd ? { additionalDirectories: [input.cwd] } : {}),
        ...(Object.keys(extraArgs).length > 0 ? { extraArgs } : {}),
        stderr: (data: string) => {
          logEvent("claude.sdk.stderr", { threadId, data: data.slice(0, 4096) });
        },
      };

      logEvent("claude.session.start.begin", {
        threadId,
        cwd: input.cwd ?? null,
        ...
        hasResumeSession: !!existingResumeSessionId,
        hasIssuanceToken: !!queryOptions.env?.CLAUDE_OAT_ISSUANCE_TOKEN,
      ...
      const queryRuntime = yield* Effect.try({
        try: () =>
          ...
          logEvent("claude.session.start.fail", {
            threadId,
            durationMs: Date.now() - claudeStartedAt,
            traceId: launchEnv.traceId,
      ...
      logEvent("claude.session.start.ok", {
        threadId,
        durationMs: Date.now() - claudeStartedAt,
      });
      ...
      startStallWatchdog(context, runFork);
  ...
  const sendTurn: ClaudeAdapterShape["sendTurn"] = Effect.fn("sendTurn")(function* (input) {
    ...
    //
    // Skip the round-trip when the requested mode matches what's already
    // applied — query.setPermissionMode writes a control message to
    // claude's stdin and awaits a control response. On a fresh session
    // claude can be late to ack the first control message, which hangs
    // sendTurn before it reaches the user-message Queue.offer below.
    // The first turn always carries interactionMode="default" with the
    // session already in basePermissionMode, so this is a guaranteed
    // no-op we can drop on the floor without changing semantics.
    const desiredPermissionMode =
      input.interactionMode === "plan"
        ...
        : input.interactionMode === "default"
          ...
          : null;
    if (desiredPermissionMode !== null && desiredPermissionMode !== context.currentPermissionMode) {
      logEvent("claude.turn.permissionMode.set", {
        threadId: input.threadId,
        from: context.currentPermissionMode ?? null,
        to: desiredPermissionMode,
      });
      yield* Effect.tryPromise({
    ...
    const queueSizeBefore = yield* Queue.size(context.promptQueue);
    logEvent("claude.turn.queue.before", {
      threadId: input.threadId,
      turnId: String(turnId),
      queueSize: queueSizeBefore,
      promptBytes: typeof input.input === "string" ? Buffer.byteLength(input.input, "utf8") : 0,
    ...
    yield* Queue.offer(context.promptQueue, {
      ...
      Effect.tapError((cause) =>
        Effect.sync(() =>
          logEvent("claude.turn.queue.fail", {
            threadId: input.threadId,
            turnId: String(turnId),
            cause: String(cause),
          }),
    ...
    const queueSizeAfter = yield* Queue.size(context.promptQueue);
    logEvent("claude.turn.queue.ok", {
      threadId: input.threadId,
      turnId: String(turnId),
      queueSize: queueSizeAfter,
      hasStreamFiber: !!context.streamFiber,
