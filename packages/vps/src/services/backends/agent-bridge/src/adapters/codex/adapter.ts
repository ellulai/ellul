...
// pingdotgg/t3code@b0b7b38 apps/server/src/provider/Layers/CodexAdapter.ts
...
const _CODEX_SOFT_HINT_MB = _computeBudget(Math.max(512, Math.round(_os.totalmem() / (1024 * 1024)))).perSandboxSoftHintMB;
let _codexScopeGen = 0;
import { Context, Effect, Exit, Fiber, FileSystem, Layer, Queue, Schema, Scope, Stream } from "effect";
      ...
      Effect.gen(function* () {
        ...
        const runtimeInput: CodexSessionRuntimeOptions = {
          ...
          ...(input.modelSelection?.provider === "codex" && input.modelSelection.options?.fastMode
            ? { serviceTier: "fast" }
            : {}),
          // ellul-only: set the namespace project env so every Command
          // reaching NamespaceChildProcessSpawner carries the sandbox id.
          // Spawner fails closed if this is missing or unparseable.
          additionalEnv: (() => {
            try {
              return {
                [NAMESPACE_PROJECT_ENV]: sandboxIdFromCwd(input.cwd),
                [NAMESPACE_ADAPTER_ENV]: "codex",
                [NAMESPACE_SCOPE_ID_ENV]: `g${++_codexScopeGen}`,
                [NAMESPACE_SOFT_HINT_MB_ENV]: String(_CODEX_SOFT_HINT_MB),
              };
            } catch (cause) {
              throw new ProviderAdapterValidationError({
                provider: PROVIDER,
                operation: "startSession",
                issue: `Codex startSession requires a sandbox-scoped cwd so the process enters its namespace. Received: ${input.cwd ?? "<none>"}`,
                cause,
              });
            }
          })(),
...
// Convenience: build the adapter as a plain Effect (like the skeleton did) so
// runtime.ts can consume it without a Layer when appropriate.
export const makeCodexAdapterShape = (options?: CodexAdapterLiveOptions) =>
  makeCodexAdapter(options);
