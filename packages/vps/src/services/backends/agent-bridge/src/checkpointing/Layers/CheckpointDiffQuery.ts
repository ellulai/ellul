...
// pingdotgg/t3code@b0b7b38 apps/server/src/checkpointing/Layers/CheckpointDiffQuery.ts
...
import { CheckpointInvariantError, CheckpointUnavailableError } from "../Errors";
import {
  CheckpointDiffQuery,
  type CheckpointDiffQueryShape,
} from "../Services/CheckpointDiffQuery";
import { CheckpointStore } from "../Services/CheckpointStore";
import { checkpointRefForThreadTurn } from "../Utils";
  ...
  const getTurnDiff: CheckpointDiffQueryShape["getTurnDiff"] = Effect.fn("getTurnDiff")(
    function* (input) {
      ...
      const [fromExists, toExists] = yield* Effect.all(
        [
          checkpointStore.hasCheckpointRef({ cwd: workspaceCwd, checkpointRef: fromCheckpointRef }),
          checkpointStore.hasCheckpointRef({ cwd: workspaceCwd, checkpointRef: toCheckpointRef }),
        ],
