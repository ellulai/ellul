...
// pingdotgg/t3code@b0b7b38 apps/server/src/orchestration/Layers/ProjectionSnapshotQuery.ts
...
import { ProjectionCheckpoint } from "../../persistence/Services/ProjectionCheckpoints";
import { ProjectionProject } from "../../persistence/Services/ProjectionProjects";
import { ProjectionSandboxPinnedModel } from "../../persistence/Services/ProjectionSandbox";
import { ProjectionState } from "../../persistence/Services/ProjectionState";
import { ProjectionThreadActivity } from "../../persistence/Services/ProjectionThreadActivities";
import { ProjectionThreadMessage } from "../../persistence/Services/ProjectionThreadMessages";
import { ProjectionThreadProposedPlan } from "../../persistence/Services/ProjectionThreadProposedPlans";
import { ProjectionThreadSession } from "../../persistence/Services/ProjectionThreadSessions";
import { ProjectionThread } from "../../persistence/Services/ProjectionThreads";
import { RepositoryIdentityResolver } from "../../project/Services/RepositoryIdentityResolver";
import { ORCHESTRATION_PROJECTOR_NAMES } from "./ProjectionPipeline";
import {
...
const REQUIRED_SNAPSHOT_PROJECTORS = [
  ...
  ORCHESTRATION_PROJECTOR_NAMES.sandbox,
] as const;
...
const makeProjectionSnapshotQuery = Effect.gen(function* () {
  ...
  const listSandboxPinnedModelRows = SqlSchema.findAll({
    Request: Schema.Void,
  ...
  const buildSandbox = (
    rows: ReadonlyArray<Schema.Schema.Type<typeof ProjectionSandboxPinnedModel>>,
    fallbackUpdatedAt: string,
  ): OrchestrationSandbox => {
    const pinnedModelByProvider: {
      -readonly [K in keyof OrchestrationSandbox["pinnedModelByProvider"]]?: OrchestrationSandbox["pinnedModelByProvider"][K];
    } = {};
    let latestUpdatedAt: string | null = null;
    for (const row of rows) {
      pinnedModelByProvider[row.provider] = row.modelSlug;
      latestUpdatedAt = maxIso(latestUpdatedAt, row.updatedAt);
    }
    return {
      pinnedModelByProvider,
      updatedAt: latestUpdatedAt ?? fallbackUpdatedAt,
    };
  };
    ...
    sql
      .withTransaction(
        Effect.all([
          ...
          listSandboxPinnedModelRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getSnapshot:listSandbox:query",
                "ProjectionSnapshotQuery.getSnapshot:listSandbox:decodeRows",
              ),
            ),
          ),
        ]),
        ...
        Effect.flatMap(
          ([
            ...
            Effect.gen(function* () {
              ...
              const finalUpdatedAt = updatedAt ?? new Date(0).toISOString();
              const sandbox = buildSandbox(sandboxRows, finalUpdatedAt);
  ...
  const getShellSnapshot: ProjectionSnapshotQueryShape["getShellSnapshot"] = () =>
    sql
      .withTransaction(
        Effect.all([
          ...
          listSandboxPinnedModelRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getShellSnapshot:listSandbox:query",
                "ProjectionSnapshotQuery.getShellSnapshot:listSandbox:decodeRows",
              ),
            ),
          ),
        ]),
      ...
      .pipe(
        Effect.flatMap(
          ([projectRows, threadRows, sessionRows, latestTurnRows, stateRows, sandboxRows]) =>
            Effect.gen(function* () {
              let updatedAt: string | null = null;
              for (const row of projectRows) {
                updatedAt = maxIso(updatedAt, row.updatedAt);
            ...
            const snapshot = {
              ...
              sandbox: buildSandbox(sandboxRows, updatedAt ?? new Date(0).toISOString()),
  ...
  const getThreadShellById: ProjectionSnapshotQueryShape["getThreadShellById"] = (threadId) =>
    Effect.gen(function* () {
      ...
      return Option.some({
        ...
        viewScope: threadRow.value.viewScope,
