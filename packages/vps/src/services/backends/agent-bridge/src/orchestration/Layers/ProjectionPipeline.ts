...
// pingdotgg/t3code@b0b7b38 apps/server/src/orchestration/Layers/ProjectionPipeline.ts
...
import { toPersistenceSqlError, type ProjectionRepositoryError } from "../../persistence/Errors";
import { OrchestrationEventStore } from "../../persistence/Services/OrchestrationEventStore";
import { ProjectionPendingApprovalRepository } from "../../persistence/Services/ProjectionPendingApprovals";
import { ProjectionProjectRepository } from "../../persistence/Services/ProjectionProjects";
import { ProjectionSandboxPinnedModelRepository } from "../../persistence/Services/ProjectionSandbox";
import { ProjectionStateRepository } from "../../persistence/Services/ProjectionState";
import { ProjectionThreadActivityRepository } from "../../persistence/Services/ProjectionThreadActivities";
import { type ProjectionThreadActivity } from "../../persistence/Services/ProjectionThreadActivities";
import {
...
} from "../../persistence/Services/ProjectionThreadProposedPlans";
...
import { ProjectionPendingApprovalRepositoryLive } from "../../persistence/Layers/ProjectionPendingApprovals";
import { ProjectionProjectRepositoryLive } from "../../persistence/Layers/ProjectionProjects";
import { ProjectionSandboxPinnedModelRepositoryLive } from "../../persistence/Layers/ProjectionSandbox";
...
import { ProjectionThreadProposedPlanRepositoryLive } from "../../persistence/Layers/ProjectionThreadProposedPlans";
...
} from "../Services/ProjectionPipeline";
import {
  attachmentRelativePath,
  parseAttachmentIdFromRelativePath,
  parseThreadSegmentFromAttachmentId,
  toSafeThreadAttachmentSegment,
} from "../../shared/attachmentStore";
import { triggerImmediateReconcile } from "../../application/namespace/NamespaceLifecycle";
import { sloEnd as sloEndSpan } from "../../application/slo/SloTelemetry";
...
const makeOrchestrationProjectionPipeline = Effect.fn("makeOrchestrationProjectionPipeline")(
  function* () {
    ...
    const projectionSandboxPinnedModelRepository = yield* ProjectionSandboxPinnedModelRepository;
    ...
    const applyProjectsProjection: ProjectorDefinition["apply"] = Effect.fn(
      ...
      switch (event.type) {
        case "project.created":
          ...
          // Pre-warm the namespace in the background so the cold setup
          // is paid before the first thread send.
          yield* Effect.sync(() => triggerImmediateReconcile());
          return;
      ...
      switch (event.type) {
        case "thread.created":
          yield* projectionThreadRepository.upsert({
            ...
            viewScope: event.payload.viewScope,
            latestTurnId: null,
            createdAt: event.payload.createdAt,
            updatedAt: event.payload.updatedAt,
            archivedAt: null,
          ...
          // Close the SLO span opened in the dispatch handler.
          yield* Effect.sync(() =>
            sloEndSpan("thread-create", event.payload.threadId, {
              projectId: event.payload.projectId,
              runtimeMode: event.payload.runtimeMode,
              source: "projector.thread.created",
            }),
          );
          return;
        ...
        case "thread.user-input-response-requested": {
          ...
          // Close the SLO first-byte span on the first activity append.
          if (event.type === "thread.activity-appended") {
            yield* Effect.sync(() =>
              sloEndSpan("first-byte", event.payload.threadId, {
                source: "projector.thread.activity-appended",
              }),
            );
          }
          return;
    ...
    const applySandboxProjection: ProjectorDefinition["apply"] = Effect.fn(
      "applySandboxProjection",
    )(function* (event, _attachmentSideEffects) {
      switch (event.type) {
        case "sandbox.model-pinned":
          yield* projectionSandboxPinnedModelRepository.upsert({
            provider: event.payload.provider,
            modelSlug: event.payload.model,
            updatedAt: event.payload.updatedAt,
          });
          return;

        case "sandbox.model-unpinned":
          yield* projectionSandboxPinnedModelRepository.deleteByProvider({
            provider: event.payload.provider,
          });
          return;
    ...
    const projectors: ReadonlyArray<ProjectorDefinition> = [
      ...
      {
        name: ORCHESTRATION_PROJECTOR_NAMES.sandbox,
        apply: applySandboxProjection,
      },
    ];
...
export const OrchestrationProjectionPipelineLive = Layer.effect(
  ...
  Layer.provideMerge(ProjectionSandboxPinnedModelRepositoryLive),
