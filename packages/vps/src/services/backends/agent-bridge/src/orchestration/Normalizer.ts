...
// pingdotgg/t3code@b0b7b38 apps/server/src/orchestration/Normalizer.ts
...
import { createAttachmentId, resolveAttachmentPath } from "../shared/attachmentStore";
import { parseBase64DataUrl } from "../shared/imageMime";
...
export const normalizeDispatchCommand = (command: ClientOrchestrationCommand) =>
  Effect.gen(function* () {
    ...
    // worktreePath overrides project.workspaceRoot in resolveThreadWorkspaceCwd, so it must be containment-checked too.
    if (
      ...
      return {
        ...command,
        worktreePath: yield* normalizeProjectWorkspaceRoot(command.worktreePath),
      } satisfies OrchestrationCommand;
    }
