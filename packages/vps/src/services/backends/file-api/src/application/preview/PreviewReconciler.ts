...
export async function reconcileOnce(log: ReconcilerLog): Promise<ReconcileReport> {
  ...
  // ── 1. Failed-unit GC ───────────────────────────────────────
  try {
    const failed = previewPlatform.listFailedUnits();
    ...
    log('warn', 'reconciler: listFailedUnits failed', { error: (err as Error).message });
  }
...
export const escapeInstance = (dir: string) => previewPlatform.escapeInstance(dir);
