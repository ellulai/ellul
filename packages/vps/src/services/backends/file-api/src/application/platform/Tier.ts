...
export function getCurrentTier(): 'standard' | 'web_locked' | 'private_locked' {
  // SECURITY: Check web_locked marker FIRST — fail-secure.
  // This prevents tier file corruption/deletion from downgrading security.
  try {
    const tierFile = fs.readFileSync(PATHS.TIER, 'utf8').trim();
    if (tierFile === 'private_locked') return 'private_locked';
  } catch {}
  if (fs.existsSync('/etc/ellul/.sovereign-shield-active')) { return 'web_locked'; }
  try {
    ...
    // Tier file missing or unreadable — marker already checked above.
  }
  return 'standard';
}
...
export async function executeTierSwitch(
  ...
  console.log(`[file-api] Delegating tier switch to shield: ${currentTier} -> ${targetTier}`);

  const { callShieldInternal } = await import('@vps/shared/shield-client');
  const response = await callShieldInternal('/_auth/tier/switch', {
    method: 'POST',
    body: {
      targetTier,
      source: 'file-api',
  ...
  console.log(`[file-api] Tier switch completed via shield: ${currentTier} -> ${targetTier}`);
}
