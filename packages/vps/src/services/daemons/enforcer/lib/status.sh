...
get_deployed_apps() {
  local APPS_DIR="${SVC_HOME}/.ellul/apps"
...
# Get disk usage percentage for a given mount point (default: root /)
get_disk_usage() {
  local MOUNT="${1:-/}"
  df "$MOUNT" 2>/dev/null | awk 'NR==2 {gsub(/%/, "", $5); print $5}'
}
