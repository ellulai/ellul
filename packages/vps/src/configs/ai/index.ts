...
export interface WaterfallModelConfig {
  id: string;
  name: string;
  provider: "opencode" | "deepseek" | "openrouter" | "anthropic" | "openai";
  modelId: string;
  baseUrl: string;
  isPaid: boolean;
  description: string;
...
// ZeroClaw config generation moved to packages/vps/src/configs/zeroclaw/index.ts
// (TOML format via getZeroClawConfigToml / getZeroClawConfigTomlWithPlaceholders)
