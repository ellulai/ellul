...
if [ -z "$NAME" ]; then
  ...
  echo ""
  echo "Usage: ellul-expose <app_name> [port] [custom_domain]"
  echo ""
  echo "Examples:"
  echo "  ellul-expose blog"
  echo "  ellul-expose api 4000"
  echo "  ellul-expose shop 3001 shop.example.com"
  echo ""
  exit 1
fi
...
if [ -z "$DEPLOY_TOKEN" ]; then
  echo ""
  echo -e "${RED}Deploy not authorized${NC}"
  echo ""
  echo "Deployment requires authorization from the console UI."
  echo "Ask the user to click the Deploy button when they are ready."
  echo ""
  exit 1
fi

__STACK_DETECT_BLOCK__

# ── Send to privileged service ────────────────────────────────────
BODY="{\"name\":\"$NAME\",\"port\":$PORT,\"deployToken\":\"$DEPLOY_TOKEN\""
if [ -n "$CUSTOM_DOMAIN" ]; then
  BODY="$BODY,\"customDomain\":\"$CUSTOM_DOMAIN\""
fi
BODY="$BODY,\"projectPath\":\"$PROJECT_PATH\",\"stack\":\"$STACK\"}"

RESULT=$(curl -sf --max-time 900 -X POST http://localhost:3005/api/workflow/expose \
