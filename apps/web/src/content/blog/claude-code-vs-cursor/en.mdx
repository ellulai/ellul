...
We built [Ellul](/) because step two was missing. Ellul is your agent's computer. You bring Claude Code, Codex, Cursor's CLI, or OpenCode. We give the agent a workstation that never closes, multiple parallel sessions on isolated sandboxes, and a passkey gate that pauses every privileged action (git push, deploy, database write) until you tap to approve.
...
<Cta
  ...
  body="Ellul runs Claude Code, Codex, Cursor's CLI, and OpenCode on a workstation that doesn't sleep. Twenty dollars a month for Hobby, fifty for Pro."
