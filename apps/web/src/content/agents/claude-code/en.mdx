...
2. **Provision a workstation.** A persistent sandbox boots in roughly ninety seconds, pre-loaded with Claude Code, Codex, OpenCode, and Cursor's CLI.
...
- **Anthropic-managed cloud.** If you specifically want Anthropic to host the agent, that is their cloud, not ours. Ellul is model-agnostic and runs Codex, Cursor's CLI, and OpenCode on the same primitive.
</Callout>
