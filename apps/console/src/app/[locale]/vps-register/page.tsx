...
function VpsRegisterContent() {
  const [status, setStatus] = useState<"loading" | "registering" | "done" | "error">("loading");
      ...
      try {
        ...
        for (let i = 0; i < 30; i++) {
          const res = await fetch(
            `${API_URL}/api/auth/native/vps-auth-data?flow_id=${flowId}&type=reg_options`
          );
          const data = await res.json();
          if (data.status === "complete") {
            options = data.data as PublicKeyCredentialCreationOptionsJSON;
            break;
          }
        ...
        if (!options || cancelled) {
          if (!cancelled) {
            setError("Timeout waiting for registration options");
            setStatus("error");
          }
          return;
        }
        ...
        await fetch(`${API_URL}/api/auth/native/vps-auth-data`, {
          ...
          body: JSON.stringify({ flowId, type: "attestation", data: attestation }),
        });
          ...
          <CardContent className="pt-6 text-center space-y-4">
            ...
            <div>
              ...
              <p className="text-sm text-cream/50 mt-1">
                Use Touch ID or your security key to create a passkey.
              </p>
            </div>
          </CardContent>
        </Card>
  ...
  return (
    <Shell>
      <LoadingScreen message="Preparing registration..." />
    </Shell>
  );
}
